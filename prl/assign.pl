...
#$Header: /usr/local/ollincvs/Codes/OllinAxis-BiB/prl/assign.pl,v 1.4 2019/11/28 18:59:26 malcubi Exp $
      ...
      if ($line =~ /integer/i) {
         ...
         if (($line =~ /::/) && ($line =~ /=/)) {
	       ...
	       $low  = $2;
               $high = $3;
               for ($i=1; $i<=3; $i=$i+1) {
                  print OUTFILE  "  if (var=='",$var,"(",$i,")') then\n";
                  print OUTFILE  "     if (nvalues>1) goto 200\n";
                  print OUTFILE  "     type = 'INTEGER'\n";
                  print OUTFILE  "     read(value,*,ERR=100) ",$var,"(",$i,")\n";
                  print OUTFILE  "     return\n";
                  print OUTFILE  "  end if\n";
	       }
               print OUTFILE  "\n";
