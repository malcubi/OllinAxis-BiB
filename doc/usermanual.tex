...
\begin{equation}
\partial_t \alpha = - \alpha^2 f(\alpha) {\rm trK} + \beta^m \partial_m \alpha\; .
\end{equation}
...
\item[\texttt{ELL\_solver=wave}] Solver based on turning the elliptic
  equation into a wave-type hyperbolic equation and evolving this in
  fictitious time until we reach a steady state.  Slow, but quite
  robust.
