...
vpath %.pl  prl
vpath %.f90 src:src/base:src/auto:src/geometry:src/matter:src/elliptic:fakempi
vpath %.mod objs
vpath %.o objs
vpath .perl% objs
...
ifeq ($(shell which mpifort | sed -n "s:.*\/mpifort:mpifort:p"),mpifort)
MPI := mpifort
WW := COMPILING WITH MPIFORT
else
...
ifeq ($(MPI),mpifort)
FC := $(shell mpifort -show | sed "s:\([a-z0-9]*\) .*:\1:" | xargs basename)
else
...
ifeq ($(FC),g95)
FLAGS := -O3 -ffree-form -fmod=objs -funroll-loops
else
...
ifeq ($(FC),f95)
FLAGS := -O3 -ffree-form -Jobjs -m64 -DMKL_ILP64 -I${MKLROOT}/include
else
...
# Compilation flags for the current INTEL compiler "ifort".
#
# -O3                  Optimization.
...
ifeq ($(FC),ifort)
FLAGS := -O3 -qoverride-limits -free -shared-intel -nowarn -module objs
else
...
start :  hello dir perlscripts .timeparfiles .timegraph1 .timegraph2 compile link .timeend
...
.timegraph1 : $(wildcard ollingraph/ollingraph)
	@ echo "COPYING PLOTTING PACKAGE OLLINGRAPH TO SUBDIRECTORY EXE ..."
	@ echo
	@ cp ollingraph/ollingraph exe
	@ touch .timegraph1

.timegraph2 : $(wildcard ollingraph/ollingraph2D)
	@ cp ollingraph/ollingraph2D exe
	@ touch .timegraph2
...
ollinaxis : $(OBJS)
	...
	@ echo
	@ echo
	cd objs; $(FF) $(FLAGS) $(MODS) $(OBJS) -o ../exe/ollinaxis
	@ echo
	@ echo
